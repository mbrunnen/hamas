...
class TestMessageTransport:
    ...
    async def test_communication(self, ap):
        ...
        message1 = Message(
            sender=agent1.aid,
            content=StringContent(agent1.text),
            recipient=agent2.aid)
        message2 = Message(
            sender=agent2.aid,
            content=StringContent(agent2.text),
            recipient=agent1.aid)
    ...
    async def test_broadcast(self, ap):
        ...
        message = Message(
            sender=sender.aid,
            content=StringContent(message_text),
            routing='broadcast')
    ...
    async def test_open_conversation(self, ap):
        ...
        msg = Message(
            sender=requester.aid,
            content=StringContent('test'),
            recipient=replier.aid)
    ...
    async def test_broadcast_conversation(self, ap):
        ...
        class SecretKeeper(Agent):
            def __init__(self, *args, **kwargs):
                super(SecretKeeper, self).__init__(*args, **kwargs)
                self.my_secret = 'the earth is flat and my name is {}'.format(
                    self.aid)
        ...
        request = Message(
            sender=requester.aid,
            content=StringContent(request_text),
            recipient=None,
            routing='broadcast')
    ...
    async def test_dummy_connector(self, event_loop):
        ...
        reply = await requester.remote_process_call(
            'get_secret', timeout=timeout, recipient=replier.aid)
        assert type(requester) is Agent
        assert type(replier) is SecretKeeper
    ...
    async def test_dummy_broadcast(self, event_loop):
        ...
        class SecretKeeper(Agent):
            def __init__(self, *args, **kwargs):
                super(SecretKeeper, self).__init__(*args, **kwargs)
                self.my_secret = 'the earth is flat and my aid is {}'.format(
                    self.aid)
        ...
        secrets = await requester.remote_process_call(
            'get_secret', timeout=1, routing='broadcast')
        assert 'the earth is flat and my aid is {}'.format(
            replier1.aid) in secrets
        assert 'the earth is flat and my aid is {}'.format(
            replier2.aid) in secrets
