...
from hamas import Agent, provide
from hamas import Message
from hamas.transport import RemoteProcessCall, RemoteProcessReply, StringContent
...
class TestAgent:
    ...
    async def test_create_task(self, mts):
        class MyAgent(Agent):
            def __init__(self, n_jobs, *args, **kwargs):
                ...
                self.results = []
                self.jobs = [asyncio.ensure_future(self.my_coro(i)) for i in
                             range(n_jobs)]
    ...
    async def test_receive_in_q(self, mts):
        ...
        timeout = 1
        conv_id = uuid.uuid4()
        reply_fut = agent._queues.new_queue(conv_id, agent._message_handler(conv_id, timeout, 1))
    ...
    async def test_receive_rp_call_provided(self, mts):
        ...
        with unittest.mock.patch.object(agent, 'send', make_coroutine(mock)):
            ...
            await agent.receive(msg)
            reply_msg = Message(performative='inform',
                                sender=agent.aid,
    ...
    async def test_receive_rp_call_unprovided(self, mts):
        ...
        with unittest.mock.patch.object(agent, 'send', make_coroutine(mock)):
            ...
            await agent.receive(msg)
            content = StringContent("Agent {} don't provide function {}.".format(agent.aid, function))
            reply_msg = Message(performative='refuse',
                                sender=agent.aid,
                                content=content,
                                recipient=sender,
                                routing='unicast',
    ...
    async def test_receive_rp_call_unexistent(self, mts):
        ...
        with unittest.mock.patch.object(agent, 'send', make_coroutine(mock)):
            ...
            await agent.receive(msg)
            content = StringContent("Agent {} don't has function {}.".format(agent.aid, function))
            reply_msg = Message(performative='refuse',
                                sender=agent.aid,
                                content=content,
                                recipient=sender,
    ...
    async def test_receive_belated_rp_reply(self, mts):
        agent = Agent(mts, mts.machine_name + '/1', )
        with unittest.mock.patch('hamas.management.agents.log.warning') as mock:
            sender = 'someone'
            ...
            await agent.receive(msg)
            mock.assert_called_once_with("Got an unexpected reply. Perhaps timed out.\n\tRemote function reply for 'get_aid' returned stuff.")
    ...
    async def test_custom_contents_cb(self, mts):
        ...
        agent = Agent(mts, mts.machine_name + '/1')
        with unittest.mock.patch.object(agent, 'custom_contents_cb', make_coro(cb_mock)):
            sender = 'someone'
            content = StringContent("stuff")
            msg = Message(sender=sender,
                          content=content,
