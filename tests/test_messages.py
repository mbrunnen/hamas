...
class TestMessage:
    def test_equality(self):
        args = [a for a in itertools.product(['sender', 'other sender'],
                                             [StringContent('stuff'), StringContent('other stuff')],
                                             [None, 'recipient', 'other recipient', ['recipient', 'other recipient']],
    ...
    @pytest.mark.parametrize('sender, content, recipient, routing, conv_id, performative',
                             itertools.product(['sender', 'other sender'],
                                               ...
                                               ['inform', None]))
    def test_serialization(self, sender, content, recipient, routing, conv_id, performative):
        """Test the serialization of Messages.
        """
        message = Message(sender=sender, content=content, recipient=recipient, routing=routing,
        ...
        print("Length of the serialized Message is {:d}".format(len(serialized)))
        ...
        print("Difference: Pickled - Serialized = {:d}".format(len(pickled) - len(serialized)))
        print("Ratio: Serialiazed/Pickled * 100% = {:.2%}".format(len(serialized) / len(pickled)))
